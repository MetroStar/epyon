...
if [ -f "$DASHBOARD_GENERATOR" ]; then
    ...
    if SCAN_DIR="$SCAN_DIR" TARGET_DIR="$TARGET_DIR" "$DASHBOARD_GENERATOR"; then
        ...
        echo -e "${PURPLE}📝 Generating executive summary...${NC}"
        FINDINGS_SUMMARY="$SCAN_DIR/security-findings-summary.json"
        EXECUTIVE_SUMMARY="$UNIFIED_DIR/markdown-reports/executive-summary.md"
        
        if [[ -f "$FINDINGS_SUMMARY" ]]; then
            # Extract counts from security findings summary
            ...
            cat > "$EXECUTIVE_SUMMARY" << 'EXEC_EOF'
...
1. Review the [interactive dashboard](../dashboards/security-dashboard.html) for detailed findings
2. Prioritize remediation based on severity levels
3. Check individual tool reports for specific recommendations
4. Review suppressed findings if applicable

---
*Generated by Epyon Security Scanner*
EXEC_EOF
            
            # Replace placeholders
            sed -i "s/CRITICAL_PLACEHOLDER/$CRITICAL/g" "$EXECUTIVE_SUMMARY"
            sed -i "s/HIGH_PLACEHOLDER/$HIGH/g" "$EXECUTIVE_SUMMARY"
            sed -i "s/MEDIUM_PLACEHOLDER/$MEDIUM/g" "$EXECUTIVE_SUMMARY"
            sed -i "s/LOW_PLACEHOLDER/$LOW/g" "$EXECUTIVE_SUMMARY"
            sed -i "s/TOTAL_PLACEHOLDER/$TOTAL/g" "$EXECUTIVE_SUMMARY"
            
            # Add security posture assessment
            if [[ $CRITICAL -gt 0 ]]; then
                POSTURE="🔴 **CRITICAL**: Immediate action required. $CRITICAL critical vulnerabilities detected."
            elif [[ $HIGH -gt 10 ]]; then
                POSTURE="🟠 **HIGH RISK**: Significant security concerns. $HIGH high-severity vulnerabilities detected."
            elif [[ $HIGH -gt 0 ]]; then
                POSTURE="🟡 **MODERATE RISK**: Some security issues present. $HIGH high-severity vulnerabilities detected."
            elif [[ $MEDIUM -gt 0 || $LOW -gt 0 ]]; then
                POSTURE="🟢 **LOW RISK**: No critical or high-severity issues. Regular monitoring recommended."
            else
                POSTURE="✅ **CLEAN**: No vulnerabilities detected in scan."
            fi
            
            sed -i "s/POSTURE_PLACEHOLDER/$POSTURE/" "$EXECUTIVE_SUMMARY"
            
            echo -e "${GREEN}✓ Executive summary generated: $EXECUTIVE_SUMMARY${NC}"
        else
            echo -e "${YELLOW}⚠️  Security findings summary not found, skipping executive summary generation${NC}"
        fi
    else
