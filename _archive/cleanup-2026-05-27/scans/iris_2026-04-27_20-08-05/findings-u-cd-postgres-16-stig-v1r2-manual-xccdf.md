...
Evidence:
- Static repository review completed on 2026-04-27.
- Unique user identification and authentication is enforced via Keycloak integration in api/app/auth.py (see JWTVerifier and get_current_user).
- The pg_hba.conf and database-level user uniqueness cannot be confirmed from code alone; however, the application is designed to use Keycloak for all user authentication.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The codebase includes database schema checks and uses psycopg2 for queries (see pointcloud-project/test_pointcloud.py), but does not show explicit input validation or constraints in the code provided.
- Application code (api/app/auth.py, api/app/reference_docs/services.py) does not demonstrate explicit input validation or use of prepared statements for SQL queries.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No use of dynamic code execution (e.g., eval, exec, dynamic SQL) is present in api/app/auth.py or api/app/reference_docs/services.py.
- All SQL queries in pointcloud-project/test_pointcloud.py are static and parameterless or use string formatting for table/column names only.
- No evidence of user input being used in dynamic code execution.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No dynamic code execution is present in the application source code (api/app/auth.py, api/app/reference_docs/services.py, pointcloud-project/test_pointcloud.py).
- This control is not applicable as there is no dynamic code execution to protect.
