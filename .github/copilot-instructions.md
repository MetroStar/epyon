...
- **STIG state**: `stig-results-{slug}.json` → `{ "assessments": { "APSC-DV-XXXXXX": { "status", "evidence", "confidence" } }, "token_usage": {} }`. The markdown findings file is **output only** — never read back by the scanner.
