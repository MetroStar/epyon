...
" 2>/dev/null || true
      ...
      # If required custom fields are missing, patch with placeholders and retry
      elif [[ "${create_http}" == "400" ]] && jq -e '.errors | keys[] | startswith("customfield_")' /tmp/jira_cve_create.json >/dev/null 2>&1; then
        echo "  ⚠️  ${cve_id} — required custom field(s) missing, patching with placeholders"
        # Build merged payload: read base payload + patch, merge, write to custom file
        python3 - /tmp/jira_cve_payload.json /tmp/jira_cve_create.json /tmp/jira_cve_payload_custom.json <<'MERGE_PYEOF'
import json, sys
payload_file, err_file, out_file = sys.argv[1], sys.argv[2], sys.argv[3]
with open(payload_file) as f: p = json.load(f)
with open(err_file) as f:    errs = json.load(f).get('errors', {})
adf = {"version": 1, "type": "doc", "content": [{"type": "paragraph", "content": [
    {"type": "text", "text": "N/A — auto-generated by Epyon Security Scanner"}]}]}
for k, msg in errs.items():
    if not k.startswith('customfield_'):
        continue
    m = msg.lower()
    if 'paragraph' in m or 'rich' in m or 'doc' in m:
        p['fields'][k] = adf
    else:
        p['fields'][k] = 'N/A — auto-generated by Epyon Security Scanner'
with open(out_file, 'w') as f: json.dump(p, f)
MERGE_PYEOF
        create_http=$(curl -s -o /tmp/jira_cve_create.json -w "%{http_code}" \
          -X POST \
          -H "Authorization: Basic ${AUTH}" \
          -H "Content-Type: application/json" \
          -H "Accept: application/json" \
          --data @/tmp/jira_cve_payload_custom.json \
          "${JIRA_URL}/rest/api/3/issue")
        if [[ "${create_http}" == "201" ]]; then
          local new_cve_key
          new_cve_key=$(jq -r '.key' /tmp/jira_cve_create.json)
          echo "  ✅  ${cve_id} → ${new_cve_key} (with custom field defaults)"
          echo "- \`${cve_id}\` → [${new_cve_key}](${JIRA_URL}/browse/${new_cve_key}) (${sev_label})" >> "${GITHUB_STEP_SUMMARY}"
          python3 -c "
import json
with open('/tmp/epyon_cve_map_current.json') as f: m=json.load(f)
