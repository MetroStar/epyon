...
| JIRA sync (CI) | `scripts/shell/sync-jira-findings.py` | Standalone script for JIRA reconciliation from GitHub Actions. Uses `jira_client.py` module for consistent fingerprinting. Compares current vs previous scan, creates tickets for new findings, closes tickets for remediated findings. Runs automatically in CI after full/nightly scans when JIRA credentials are configured. Skips gracefully if credentials missing or only one scan exists. |
