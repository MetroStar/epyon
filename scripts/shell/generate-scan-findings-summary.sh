...
EOF
                ...
                if [[ $verified_count -gt 0 ]]; then
                    local critical_findings=$(echo "$verified_secrets" | jq --arg tool "TruffleHog" --arg scan_id "$scan_id" '
                        [.[] | {
                            ...
                            file_path: (.SourceMetadata.Data.Filesystem.file | sub("^/workspace/"; "")),
                ...
                if [[ $github_count -gt 0 ]]; then
                    local github_findings=$(echo "$github_secrets" | jq --arg tool "TruffleHog" '
                        [.[] | {
                            ...
                            severity: "Medium",
                            detector: .DetectorName,
                            file: (.SourceMetadata.Data.Filesystem.file | sub("^/workspace/"; "")),
    ...
    # NOTE: ML findings (layers 14, 18, 19, 20) are intentionally excluded from
    # security-findings-summary.json. They are displayed in the ML/AI Security card
    # and are NOT synced to Jira as vulnerability findings.
    
    # Layer 18 — Model Provenance & Threat Intelligence
    # Excluded from security-findings-summary.json (see Layer 14 note above)
