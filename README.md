...
**Triage note (per scan):** the same modal has an optional "Triage note" field — enter your name and Epyon posts a Jira comment on every newly created ticket in that batch: `Triaged and added by: <name> on: <date>` (the date is filled in automatically). Leaving it blank creates tickets with no extra comment. Tickets that already existed are never touched.
...
### 🌐 Multi-Feed CVE Enrichment

**International Vulnerability Feed Coverage:**

Epyon enriches CVE findings from **seven global vulnerability feeds** for comprehensive security intelligence:

| Feed | Coverage | Status |
|------|----------|--------|
| **NVD** (NIST) | CVSS scores, CWE IDs, CPEs | ✅ Active |
| **CISA KEV** | Known exploited vulnerabilities | ✅ Active |
| **OSV.dev** | Open source packages (npm, PyPI, Go, Maven, RubyGems, etc.) | ✅ Active |
| **GitHub Security Advisories** | OSS ecosystem advisories with patch info | ✅ Active |
| **JVN** (Japan) | Japanese vulnerability database with international coverage | ✅ Active |
| **GitLab Advisory Database** | Git-backed OSS advisories | 🚧 Planned |
| **EUVD** (ENISA) | European Vulnerability Database | 🚧 Planned |

**Features:**
- Automatic enrichment during scans (configurable limit: 50 CVEs per scan)
- 24-hour caching to minimize API calls
- Feed-specific data: OSV summaries, GHSA severity/patch versions, JVN references
- Graceful degradation when feeds unavailable

**Configuration:** See [Multi-Feed CVE Enrichment Guide](documentation/MULTI_FEED_CVE_ENRICHMENT.md)
...
- **ML Dependency Fixtures**: pip-audit fixtures use supported package versions; adversarial typosquatting detection remains covered through mock scanner results rather than unsafe package declarations
