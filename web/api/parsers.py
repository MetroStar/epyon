"""
Scan tool output parsers — ported from web/server.js
...
def parse_trufflehog_dir(scan_dir: Path) -> list[dict]:
    ...
    for file in directory.iterdir():
        ...
        if file.name.startswith(f"{scan_id}_"):
            continue  # skip prefixed duplicate copies
        try:
...
def parse_checkov_dir(scan_dir: Path) -> list[dict]:
    ...
    for entry in entries:
        if entry.name.startswith(f"{scan_id}_"):
            continue
        try:
            # Use stat (follows symlinks) so symlinks like checkov-results.json are readable
            stat = entry.stat()
        except OSError:
            continue
