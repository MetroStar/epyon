...
- No schema or code references to classified, CUI, or other data markings are present.
...
- No code references to marking retention during processing actions.
...
- No code references to marking retention during transmission actions.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The application uses AWS S3 and Bedrock clients, but there is no explicit enforcement or verification of DoD-approved encryption (e.g., TLS configuration) in the code.
- Repository artifacts do not confirm encryption settings for remote access sessions.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The application uses AWS S3 and Bedrock clients, but there is no explicit enforcement or verification of cryptographic integrity mechanisms (e.g., TLS configuration) in the code.
- Repository artifacts do not confirm integrity protection for remote access sessions.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of SOAP message generation or processing was found in the provided codebase.
- Application does not utilize SOAP or WS-Security; only REST and AWS SDKs are present.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of WS-Security token usage or SOAP messaging was found in the provided codebase.
- Application does not utilize WS-Security or SOAP.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of WS-Security or SAML assertion usage was found in the provided codebase.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of SAML assertion usage was found in the provided codebase.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of WS-Security token usage was found in the provided codebase.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of SAML assertion or <SubjectConfirmation> element usage was found in the provided codebase.
- Application does not utilize SAML assertions.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of SAML assertion or <Conditions> element usage was found in the provided codebase.
- Application does not utilize SAML assertions.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of SAML assertion or OneTimeUse element usage was found in the provided codebase.
- Application does not utilize SAML assertions.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of SAML assertion or SessionIndex usage was found in the provided codebase.
- Application does not utilize SAML assertions.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application source code and documentation do not indicate any audit record aggregation or log correlation functionality; log aggregation is not a feature of this application.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of information category or classification level access controls or audit logging found in the provided code or documentation.
- Application requirements for compartmentalized data are not clearly stated; further review of requirements and implementation is needed.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of information category or classification level modification controls or audit logging found in the provided code or documentation.
- Application requirements for compartmentalized data are not clearly stated; further review of requirements and implementation is needed.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of information category or classification level deletion controls or audit logging found in the provided code or documentation.
- Application requirements for compartmentalized data are not clearly stated; further review of requirements and implementation is needed.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The authentication flow is implemented via Keycloak and JWT in api/app/auth.py, but there is no evidence of audit record generation for successful or unsuccessful logon attempts in the repository code.
- Logging is present for token verification, but not for authentication events.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Role-based access control is enforced in api/app/auth.py, but there is no evidence of privileged activity audit logging in the repository code.
- Logging statements are present for access attempts, but these are application logs, not audit records.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence found of audit records showing starting and ending time for user access to the system in the provided codebase.
- Session management and logging of session start/end events are not implemented in the visible code.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No direct access to the underlying operating system is provided by the application according to the supplied code and documentation.
- Application is a web-based video/document analysis tool with no OS command execution or file system navigation features.
...
Evidence:
- Static repository review completed on 2026-04-27.
- User authentication and authorization are handled via Keycloak (see api/app/auth.py and deploy/keycloak/themes/README.md), but there is no evidence of audit record generation for account creation, modification, disabling, or termination events in the application code.
- If user management is delegated entirely to Keycloak, audit logging may be handled externally; requires system-level validation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- All logging calls in api/app/process/services/thumbnail.py, api/app/process/services/summarization.py, and api/app/reference_docs/services.py use logger.info/error/warning, which typically include timestamps when properly configured in app/logging_config.py.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Logging statements in api/app/process/services/thumbnail.py and api/app/process/services/summarization.py include contextual information such as session_id, segment_index, and operation, allowing traceability to the triggering component or function.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Logging statements in api/app/process/services/thumbnail.py and api/app/process/services/summarization.py include status and message fields (e.g., 'status': 'success', 'message': 'Thumbnail generated successfully'), indicating event outcomes are recorded.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Logging and audit records in api/app/process/services/thumbnail.py and api/app/process/services/summarization.py include session_id and other identifiers, establishing the identity of the process or user associated with the event.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Logging and summary generation use datetime.utcnow() for timestamps (see api/app/process/services/summarization.py, generate_video_summary_for_session), but there is no explicit evidence that the system clock is synchronized with the OS or that UTC/GMT mapping is enforced.
- S3 object metadata may include timestamps, but system-level validation is required to confirm alignment with OS time.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Timestamps are generated using datetime.utcnow() (see api/app/process/services/summarization.py, generate_video_summary_for_session), but there is no explicit evidence that timestamps are mapped to UTC/GMT or that the system clock is configured accordingly.
- System-level configuration and validation are required to confirm UTC/GMT mapping.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Timestamps are generated using datetime.utcnow().isoformat() + 'Z' (see api/app/process/services/summarization.py), which provides ISO 8601 format with second-level granularity.
- However, there is no explicit evidence that the underlying system clock is synchronized to within one second of precision; system-level validation is required.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Audit information (e.g., summary JSON files) is stored in S3 and metadata in PostgreSQL, but there is no evidence in the repository of explicit access control enforcement for audit data at the application or infrastructure level.
- File system and S3 bucket permissions, as well as database access controls, require system-level validation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- There is no evidence in the repository of explicit controls to prevent unauthorized modification of audit information (e.g., summary files in S3 or metadata in PostgreSQL).
- Enforcement of write/modify permissions must be validated at the infrastructure and database configuration level.
...
Evidence:
- Static repository review completed on 2026-04-27.
- There is no evidence in the repository of explicit controls to prevent unauthorized deletion of audit information (e.g., summary files in S3 or metadata in PostgreSQL).
- Enforcement of delete permissions must be validated at the infrastructure and database configuration level.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No distinct audit tool functionality is present in the application codebase; audit access is via standard API/database/S3 interfaces.
- If audit tools exist outside the repository, system-level validation is required to confirm access controls.
...
- Static repository review completed on 2026-04-27.
- No distinct audit tool functionality is present in the application codebase; audit access is via standard API/database/S3 interfaces.
- If audit tools exist outside the repository, system-level validation is required to confirm modification controls.
...
- Static repository review completed on 2026-04-27.
- No distinct audit tool functionality is present in the application codebase; audit access is via standard API/database/S3 interfaces.
- If audit tools exist outside the repository, system-level validation is required to confirm deletion controls.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The application does not include a built-in backup capability for audit records; audit data (summaries, metadata) is stored in S3 and PostgreSQL, which are managed externally.
- Backup and retention are expected to be handled by infrastructure or cloud provider.
...
Evidence:
- Static repository review completed on 2026-04-27.
- There is no evidence in the repository of cryptographic mechanisms (e.g., hashing, signing) applied to audit information (summary JSON files or metadata) to protect integrity.
- S3 and database-level integrity controls, if any, require system-level validation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that implements or restricts user installation of software, plugins, or extensions within the application.
- Application source and documentation do not indicate any user-facing software installation capability.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No explicit access control logic for application configuration changes was found in the provided source code.
- Keycloak is referenced for authentication, but enforcement of configuration change restrictions is not evident from static artifacts.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of audit logging for configuration changes or user attribution was found in the repository.
- Logging is present (see api/app/process/services/summarization.py), but not for configuration changes.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces digital signature verification for patches, service packs, or application components.
- Deployment scripts (see .github/workflows/deploy-iris.yml) do not show signature validation steps.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No static evidence of file system permissions or application logic restricting modification of software libraries was found.
- Application folder and library access controls are not defined in code or documentation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces program execution restrictions based on organizational policy.
- No policy enforcement or RBAC logic for program execution is present in the reviewed files.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The application is not a configuration management or system process management tool; no evidence of application whitelisting or deny-all, permit-by-exception execution policy is present or required for this application type.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that disables or restricts non-essential capabilities or modules.
- Application configuration (api/app/config.py) lists many features, but no mechanism for disabling features is evident.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application configuration (api/app/config.py) defines ports and protocols (e.g., POSTGRES_PORT, QDRANT_PORT), but no evidence of enforcement against PPSM CAL or restriction to approved ports is present.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces user reauthentication on privilege escalation or role change.
- Authentication is handled via Keycloak, but reauthentication triggers are not evident in the repository.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces device reauthentication or periodic authenticator renewal for devices.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Unique user identification and authentication is implemented via Keycloak integration (see api/app/config.py: KEYCLOAK_URL, KEYCLOAK_REALM, KEYCLOAK_CLIENT_ID).
- Frontend authentication logic is present in ui/src/hooks/use-auth.test.ts, validating user tokens and profiles.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces multifactor (Alt. Token) authentication for privileged accounts.
- Keycloak is referenced, but MFA enforcement for privileged access is not evident in static artifacts.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces acceptance of PIV credentials (CAC) for authentication.
- Keycloak integration is present, but PIV/CAC enforcement is not statically verifiable.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that electronically verifies PIV credentials.
- Keycloak integration is present, but certificate validation for CAC/PIV is not statically verifiable.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces multifactor authentication for non-privileged accounts.
- Keycloak integration is present, but MFA enforcement is not statically verifiable.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration was found that enforces multifactor (Alt. Token) authentication for local privileged access.
- Keycloak integration is present, but local MFA enforcement is not statically verifiable.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The application uses JWT authentication with Keycloak and verifies tokens using RS256 (as seen in api/app/auth.py), which is replay-resistant if used over TLS.
- No explicit evidence of TLS enforcement for all privileged authentication traffic is present in the repository; requires deployment/runtime validation.
- No code for Kerberos, IPSEC, or SSH authentication is present.
...
Evidence:
- Static repository review completed on 2026-04-27.
- JWT authentication is implemented (api/app/auth.py) and Keycloak is used for user authentication, but static code does not confirm TLS enforcement for all nonprivileged authentication traffic.
- No explicit evidence of replay-resistant mechanisms for all network access to nonprivileged accounts; requires runtime and infra validation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No static evidence of mutual authentication (e.g., two-way SSL/TLS or client certificate validation) in the application code or configuration files.
- Keycloak is used for authentication, but mutual authentication requirements depend on Keycloak and deployment configuration, not visible in code.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration found that authenticates network-connected endpoint devices before establishing connections.
- The application uses JWT/Keycloak for user authentication, but device authentication is not addressed in the static codebase.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of Service-Oriented Architecture (SOA) or web services endpoints handling non-releasable data in the application codebase.
- Application appears to be an interactive user-facing system with no SOA mutual SSL/TLS device authentication requirements.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration found for device identifier/account management or inactivity-based disabling.
- Device authentication and inactivity tracking are not implemented in the reviewed source code.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password creation or change logic is present in the application codebase; authentication is delegated to Keycloak.
- Password policy enforcement (minimum length) is managed by Keycloak and must be validated in Keycloak configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password complexity enforcement logic is present in the application codebase; authentication is handled by Keycloak.
- Enforcement of uppercase character requirement must be validated in Keycloak configuration.
...
- Static repository review completed on 2026-04-27.
- No password complexity enforcement logic is present in the application codebase; authentication is handled by Keycloak.
- Enforcement of lowercase character requirement must be validated in Keycloak configuration.
...
- Static repository review completed on 2026-04-27.
- No password complexity enforcement logic is present in the application codebase; authentication is handled by Keycloak.
- Enforcement of numeric character requirement must be validated in Keycloak configuration.
...
- Static repository review completed on 2026-04-27.
- No password complexity enforcement logic is present in the application codebase; authentication is handled by Keycloak.
- Enforcement of special character requirement must be validated in Keycloak configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password change logic is present in the application codebase; authentication is handled by Keycloak.
- Enforcement of minimum character change on password update must be validated in Keycloak configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password storage logic is present in the application codebase; passwords are managed by Keycloak.
- Password storage and cryptographic protection must be validated in Keycloak configuration and database.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password transmission logic is present in the application codebase; authentication is handled by Keycloak.
- Cryptographic protection of passwords in transit must be validated in Keycloak and deployment configuration (e.g., HTTPS enforcement).
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password lifetime enforcement logic is present in the application codebase; authentication is handled by Keycloak.
- Minimum password lifetime enforcement must be validated in Keycloak configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password expiration logic is present in the application codebase; authentication is handled by Keycloak.
- Maximum password lifetime enforcement must be validated in Keycloak configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No password reuse prevention logic is present in the application codebase; authentication is handled by Keycloak.
- Password history enforcement must be validated in Keycloak configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No logic for temporary password issuance or forced password change on first login is present in the application codebase; authentication is handled by Keycloak.
- Temporary password and forced change enforcement must be validated in Keycloak configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No explicit certificate path validation logic is present in the reviewed source code. - PKI-based authentication and certificate validation are not implemented in the application code; likely delegated to Keycloak or external IdP. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No cryptographic private key management or access control logic is present in the application source code. - Application does not appear to perform code signing or cryptographic operations requiring private key storage. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code mapping PKI certificate data to user or group accounts is present in the application source code. - Authentication and identity mapping are likely handled by Keycloak, but repository artifacts do not demonstrate this mapping. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code for local caching of certificate revocation data (CRL/OCSP) is present in the application source code. - PKI revocation checking is not implemented in the application; likely handled by Keycloak or external IdP. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No UI code or template for password/PIN entry is present in the reviewed repository artifacts. - The Keycloak theme directory contains login templates, but the actual password field implementation and masking cannot be confirmed from static theme files alone. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No cryptographic module authentication logic is present in the application source code. - No evidence of FIPS-validated cryptographic module usage or authentication to such modules is present. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code for unique identification and authentication of non-organizational users is present in the application source code. - User authentication is delegated to Keycloak, but repository artifacts do not demonstrate user base or account uniqueness. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration for accepting PIV credentials from other federal agencies is present in the application source code. - PKI/PIV authentication is likely handled by Keycloak, but repository artifacts do not demonstrate this capability. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration for electronic verification of PIV credentials from other federal agencies is present in the application source code. - PKI/PIV authentication is likely handled by Keycloak, but repository artifacts do not demonstrate this capability. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration for accepting FICAM-approved third-party credentials is present in the application source code. - PKI/FICAM authentication is likely handled by Keycloak, but repository artifacts do not demonstrate this capability. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or configuration for conforming to FICAM-issued profiles (e.g., SAML, OpenID) is present in the application source code. - Identity federation and protocol conformance is likely handled by Keycloak, but repository artifacts do not demonstrate this capability. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code for auditing non-local maintenance or diagnostic sessions is present in the application source code. - No evidence of audit log generation for remote maintenance actions is present. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code for implementing cryptographic protection of non-local maintenance and diagnostic communications is present in the application source code. - No evidence of TLS or other encryption for remote maintenance sessions is present. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code for verifying remote disconnection at the termination of non-local maintenance and diagnostic sessions is present in the application source code. - No evidence of session termination or connection closure logic for remote maintenance is present. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code for strong authentication mechanisms for non-local maintenance and diagnostic sessions is present in the application source code. - No evidence of CAC or equivalent strong authenticator enforcement for remote maintenance is present. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code for terminating all sessions and network connections after nonlocal maintenance is completed is present in the application source code. - No evidence of session timeout or forced termination logic for privileged or nonprivileged sessions is present. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No static analysis results or explicit race condition mitigation logic is present in the application source code. - Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No SAML assertion generation or SAML SessionIndex logic is present in the provided source code or documentation.
- Application uses Keycloak for authentication, which is OIDC/OAuth2-based by default.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application uses Keycloak for authentication and session management, but no explicit evidence of HTTPOnly flag configuration for session cookies is present in the repository.
- Keycloak theme customization does not affect cookie flags; requires Keycloak server config review.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application uses Keycloak for authentication and session management, but no explicit evidence of Secure flag configuration for session cookies is present in the repository.
- Keycloak theme customization does not affect cookie flags; requires Keycloak server config review.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application relies on Keycloak for session management, but repository artifacts do not demonstrate encryption of session IDs in transit.
- Requires runtime validation of TLS configuration between client, UI, and Keycloak.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Session management is delegated to Keycloak, but repository artifacts do not demonstrate destruction of session IDs/cookies on logoff or browser close.
- Requires Keycloak server/session config review and runtime validation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Session identifier generation and fixation protection are handled by Keycloak, but no explicit evidence of anti-fixation controls is present in the repository.
- Requires review of Keycloak server configuration and/or dynamic testing.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Session validation is handled by Keycloak, but repository artifacts do not demonstrate session ID validation logic or configuration.
- Requires Keycloak server/session config review.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of URL-embedded session IDs or URL rewriting for session management in the repository.
- However, explicit configuration or code preventing URL-based session IDs is not present; requires Keycloak and UI runtime validation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Session ID generation and lifecycle are managed by Keycloak, but repository artifacts do not demonstrate controls to prevent session ID reuse or recycling.
- Requires Keycloak server/session config review and/or dynamic testing.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Session ID generation is delegated to Keycloak, but repository artifacts do not demonstrate use of FIPS 140-2/140-3 approved random number generators for session IDs.
- Requires Keycloak server configuration review and/or vendor documentation.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application uses Keycloak for authentication, but repository artifacts do not demonstrate enforcement of DoD-approved certificate authorities for protected sessions.
- Requires Keycloak server TLS/certificate configuration review.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application processes and stores data in S3 and PostgreSQL, but repository artifacts do not demonstrate encryption or integrity controls for stored information.
- Requires review of S3 bucket policies, encryption settings, and database configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Application stores data in S3 and PostgreSQL, but repository artifacts do not demonstrate use of approved cryptographic mechanisms to prevent unauthorized modification of data at rest.
- Requires review of S3 encryption settings, database encryption, and relevant policies.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No XML-based application code, XML parsing, or XML web service endpoints were identified in the provided source code or documentation; this application does not process XML.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or documentation indicates the application performs organization-defined security functions or security function testing; the application is not a security function testing platform.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or documentation indicates the application performs security function testing on startup, restart, or privileged command; the application is not a security function testing platform.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No code or documentation indicates the application performs security function testing or notifies ISSO/ISSM of failed security verification tests; the application is not a security function testing platform.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No mobile code (e.g., Java applets, ActiveX, Flash, Silverlight) is present in the repository; application is not a client-distributed mobile code platform.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No web service implementation code or documentation is present; application does not expose or develop web services, so deadlock/recursion mitigation is not applicable.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The application uses AWS Bedrock and S3, which may involve key exchange, but no explicit cryptographic key exchange implementation is visible in the provided code.
- No evidence of FIPS-140-2 validated cryptographic modules for key exchange is present in the repository.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- Sensitive data such as AWS credentials and PostgreSQL passwords are referenced in api/app/config.py as environment variables, not hardcoded values.
- No embedded authentication data (e.g., plaintext passwords, certificates) was found in the code, but full assurance requires review of all configuration and deployment artifacts.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of output marking for sensitive/classified data was found in the provided source code or theme templates.
- No classification guide or marking logic is present in the repository artifacts.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No test plans, procedures, or results were found in the repository.
- No evidence of test documentation or automation for release/patch cycles is present in the supplied codebase.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No cryptographic hash validation process or documentation was found in the repository.
- No evidence of file integrity validation prior to deployment is present in the codebase.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of designated security testers or related documentation was found in the repository.
- No organization chart or personnel assignment for security testing is present in the codebase.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No test procedures or documentation for system initialization, shutdown, or abort secure state validation were found in the repository.
- No evidence of annual execution or test results is present in the codebase.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of a formal code review process or code review reports was found in the repository.
- No documentation outlining code review procedures or results is present in the codebase.
- Control-specific implementation evidence was not demonstrably satisfied from repository artifacts alone; disposition set to Open pending system-level validation and artifact collection.
...
Evidence:
- Static repository review completed on 2026-04-27.
- ui/README.md documents the use of 'npm run test:coverage' to generate code coverage statistics for each release.
- The README instructs developers to review the coverage directory, indicating code coverage is maintained and available.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No static code analysis reports or test results for error handling vulnerabilities were found in the repository.
- Evidence of error handling exists in exception handling (e.g., try/except blocks in api/app/process/services/summarization.py), but no proof of systematic testing or scan results.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of built-in application accounts or their management was found in the repository code or documentation.
- Application appears to use Keycloak for authentication, but built-in account handling is not addressed in code artifacts.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of default password management or enforcement was found in the repository code or documentation.
- Keycloak admin credentials are documented in ui/README.md for development, but production password management is not addressed.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No Application Configuration Guide was found in the repository.
- While configuration details are present in api/app/config.py and some documentation, a comprehensive configuration guide as described in the control is not included.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence was found that the application processes classified information.
- No Security Classification Guide or references to classified data handling are present in the repository.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No documentation or code evidence was found regarding the use or prohibition of uncategorized or emerging mobile code.
- Application appears to be a web service and UI, but explicit mobile code controls are not documented.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence was found regarding the sanitization of database exports before release to test or development environments.
- No database export scripts or sanitization routines are present in the repository.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No threat model documentation or code evidence was found regarding DoS protections.
- Application code does not explicitly address DoS mitigation in repository artifacts.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of system resource monitoring or administrator alerting for low resource conditions was found in the repository code or documentation.
- No monitoring or alerting integrations are present in static artifacts.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No network or infrastructure configuration files (e.g., firewall, DMZ, or network topology) are present in the repository.
- Application code and documentation do not specify DMZ enforcement or public accessibility controls.
- Requires system/network architecture validation outside of codebase.
...
Evidence:
- Static repository review completed on 2026-04-27.
- No evidence of audit logging for concurrent logons or IP address tracking was found in the provided source code.
- No log management or session tracking implementation is visible in the reviewed files.
- Requires dynamic testing and review of authentication/audit configuration.
...
Evidence:
- Static repository review completed on 2026-04-27.
- The application references AWS GovCloud and Bedrock, but there is no evidence that classified data is processed or that NSA-approved cryptography is implemented.
- Cryptographic implementation details (e.g., FIPS/NIST/NSA module usage) are not present in the codebase.
- Requires review of system security plan, data classification, and cryptographic configuration.
