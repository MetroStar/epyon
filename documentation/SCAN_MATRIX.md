...
| Schedule | Mode | STIG |
|----------|------|------|
| Mon–Sat 02:00 UTC | `nightly` | ⬜ Skipped (not enabled) |
| Sun 02:00 UTC | `full` | ✅ Runs (`run_stig: true`) |
| Manual dispatch | Selected mode | ✅ Only if `scan_mode=stig` or `run_stig: true` |
| CI push to main | `full` | ⬜ Skipped (opt-in only) |
| CI pull request | `quick` | ⬜ Skipped (opt-in only) |
