"""
GitHub Actions artifact sync — ported from web/server.js
"""
...
async def _download_artifact(url: str, token: str, dest: Path) -> None:
    """
    Download artifact zip following redirects.
    Auth header is stripped for non-api.github.com hosts (CDN redirect fix).
    """
...
async def run_github_sync(
    ...
    """
    Download new artifacts from all configured GitHub repos.
                ...
                for run in runs_data.get("workflow_runs") or []:
                    ...
                    for artifact in arts_data.get("artifacts") or []:
                        ...
                        try:
                            ...
                            existing_ids.add(name)  # avoid re-downloading in same run
...
async def trigger_sync(
    ...
    """
    Start a sync in the background, updating _sync_state.
    Safe to call from the API endpoint without awaiting completion.
    """
    global _sync_state
